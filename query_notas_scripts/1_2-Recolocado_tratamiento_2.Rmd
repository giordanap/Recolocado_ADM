...
for (i in colnames(data)) { if ( sum( is.na( data[, colnames(data)[1]] ) ) <= dim(data)[1] ) { var_num = append(var_num, i) } }

data_woeBinning = woe.binning(data, "tm", colnames(data[, var]), min.perc.total=0.05)
...
write.table(data_wbng,paste(readline(),"data_wbng.csv",sep="\\"), sep=',')

# data_wbng = read.table(file.choose(), header = T, sep = ',')
# 
# var = as.character(data_wbng[data_wbng[,'iv'] >= 0.02 & data_wbng[,'iv'] < 0.5,]$var)
...
for (i in 1:n) {
  data_na[i, 'var'] = var[i]
  data_na[i, 'prc_na'] = length(which(is.na( data[, var[i]] )))/m
}

# data_na[data_na[,'prc_na'] > 0.8,]
# data_na[order(data_na$prc_na),]

# write.table(data_na,paste(readline(),"na.csv",sep="\\"), sep=',')

# https://rpubs.com/cghv94/operaciones-con-conjuntos
var = setdiff(var, as.character( data_na[data_na[, 'prc_na'] > 0.8, 'var'] ) )
...
# write.table(data_cv,paste(readline(),"cv.csv",sep="\\"), sep=',')
...
# write.table(data_cor,paste(readline(),"cor.csv",sep="\\"), sep=',')
...
```{r}
var_chr = c("por_cal1234_var1m3","por_mora0_var1m3","por_cal1234_max1m6_max7m12","calificacion_1m_max2m12","por_mora30_var1m3","por_mora60_var1m3","mora_var1m3","calificacion_max1m3_max4m12","mora_1m_max2m12","por_mora90_var1m3","por_cal234_var1m3","por_cal34_var1m3","por_cal4_var1m3","por_mora60_1m_max2m12","por_mora90_1m_max2m12","por_castigo_var1m3","por_mora0_max1m6_max7m12","por_cal4_1m_max2m12","por_vig_var1m3","por_cal234_1m_max2m12","por_mora30_1m_max2m12","por_castigo_1m_max2m12","por_mora_0_var1m3","por_cal34_1m_max2m12","por_cal0_var1m3","calificacion_var1m6","por_vig_1m_min2m12","saldo_var1m3","saldo_1m_max2m12","por_cal0_min1m6_min7m12","por_mora_0_1m_min2m12","vivienda","linea_activa_var1m3","por_uso_linea_var1m3","linea_activa_max1m6_max7m12","por_uso_efectivo_var1m3","por_uso_linea_max1m3_max4m12","por_uso_efectivo_max1m6_max7m12","perfil","garpref","can_emp_var1m12","can_emp_1m_max2m12","por_novig_1m_max2m12")
```

```{r}

### Train & Test
data_train = subset(data, stock >= 201301 & stock <= 201804 & filtro == 0)
data_test = subset(data, stock >= 201805 & stock <= 202003 & filtro == 0)
### OOT
data_oot = subset(data, stock > 202003 & filtro == 0)
